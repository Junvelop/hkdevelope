...
public class HelloServletOld extends HttpServlet{
	...
	public void init(ServletConfig config) throws ServletException {
		...
		//servletContext 객체 얻어오기(jsp에서 scope객체중 어플리케이션 객체를 의미함)
		ServletContext application = config.getServletContext()	;
		application.setAttribute("id", "id");//객체 저장
		
		//웹.xml에 값을 정의해놓고 가져올 수도 있다
		String driver = application.getInitParameter("driver");
		System.out.println("어플리케이션 객체에 저장된 값 :" +driver);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//forward
//		req.getRequestDispatcher("test.jsp").forwa.rd(req, resp);
	}
	
	@Override
	public void destroy() {
		System.out.println("destroy(): 더 이상 요청이 없을 경우 서블릿을 소멸시킴");
	}
